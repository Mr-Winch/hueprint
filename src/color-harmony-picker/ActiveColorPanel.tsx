"use client";

import { KeyboardEvent, useEffect, useState } from "react";
import styles from "./ColorHarmonyPicker.module.css";
import { clamp, hexToHsl, sanitizeHex, setHslLightness } from "./colorHarmony.math";
import { HarmonyRule } from "./colorHarmony.types";

type EyeDropperResult = {
  sRGBHex: string;
};

type EyeDropperConstructor = new () => {
  open: () => Promise<EyeDropperResult>;
};

type WindowWithEyeDropper = Window & {
  EyeDropper?: EyeDropperConstructor;
};

const LIGHTNESS_MIN_PERCENT = 8;
const LIGHTNESS_MAX_PERCENT = 92;

type ActiveColorPanelProps = {
  activeHex: string;
  canAddActiveColor: boolean;
  onAddActiveColor: () => void;
  onColorChange: (hex: string) => void;
  onRuleChange: (rule: HarmonyRule) => void;
};

export function ActiveColorPanel({ activeHex, canAddActiveColor, onAddActiveColor, onColorChange, onRuleChange }: ActiveColorPanelProps) {
  const [draftHex, setDraftHex] = useState(activeHex);
  const [copied, setCopied] = useState(false);
  const [canPickFromScreen, setCanPickFromScreen] = useState(false);

  useEffect(() => {
    setDraftHex(activeHex);
  }, [activeHex]);

  useEffect(() => {
    setCanPickFromScreen("EyeDropper" in window);
  }, []);

  const activeHsl = hexToHsl(activeHex);

  function commitDraft() {
    onColorChange(sanitizeHex(draftHex, activeHex));
  }

  function changeLightness(nextLightness: number) {
    onColorChange(setHslLightness(activeHex, clamp(nextLightness, LIGHTNESS_MIN_PERCENT, LIGHTNESS_MAX_PERCENT) / 100, activeHsl.h));
  }

  function handleHexKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    if (event.key !== "Enter") return;
    event.preventDefault();
    commitDraft();
  }

  async function copyHex() {
    await navigator.clipboard?.writeText(activeHex);
    setCopied(true);
    window.setTimeout(() => setCopied(false), 1100);
  }

  async function pickFromScreen() {
    const EyeDropper = (window as WindowWithEyeDropper).EyeDropper;
    if (!EyeDropper) return;

    try {
      const result = await new EyeDropper().open();
      onColorChange(result.sRGBHex);
    } catch {
      // The browser throws when the user cancels the eyedropper.
    }
  }

  return (
    <div className={styles.activePanel}>
      <div className={styles.field}>
        <label htmlFor="active-hex">Active color</label>
        <div className={styles.hexInputRow}>
          <input
            id="active-hex-color"
            className={styles.colorInput}
            type="color"
            value={activeHex}
            aria-label="Active color picker"
            onChange={(event) => onColorChange(event.target.value)}
          />
          <input
            id="active-hex"
            className={styles.hexInput}
            value={draftHex}
            onBlur={commitDraft}
            onKeyDown={handleHexKeyDown}
            onChange={(event) => {
              const value = event.target.value;
              if (/^#?[a-f\d]{0,6}$/i.test(value)) setDraftHex(value.startsWith("#") ? value : `#${value}`);
            }}
          />
          <div className={styles.lightnessControl}>
            <label htmlFor="active-lightness">Lightness</label>
            <input
              id="active-lightness"
              className={styles.lightnessRange}
              type="range"
              min={LIGHTNESS_MIN_PERCENT}
              max={LIGHTNESS_MAX_PERCENT}
              value={clamp(Math.round(activeHsl.l * 100), LIGHTNESS_MIN_PERCENT, LIGHTNESS_MAX_PERCENT)}
              onChange={(event) => changeLightness(Number(event.target.value))}
              style={{ background: `linear-gradient(90deg, #000000, ${activeHex}, #ffffff)` }}
            />
          </div>
          <button
            type="button"
            className={`${styles.iconButton} ${styles.squareIconButton}`}
            aria-label="Add active color to saved palette"
            title="Add active color"
            onClick={onAddActiveColor}
            disabled={!canAddActiveColor}
          >
            <span className={`${styles.toolIcon} ${styles.plusIcon}`} aria-hidden="true" />
          </button>
          <button
            type="button"
            className={`${styles.iconButton} ${styles.squareIconButton}`}
            aria-label="Pick color from screen"
            title={canPickFromScreen ? "Pick color from screen" : "Screen color picker not supported"}
            onClick={pickFromScreen}
            disabled={!canPickFromScreen}
          >
            <svg className={styles.svgIcon} viewBox="0 0 15 15" aria-hidden="true">
              <path d="M0.5 12.5L0.146447 12.1465L0 12.2929V12.5H0.5ZM8 5L8.35355 4.64645C8.15829 4.45119 7.84171 4.45119 7.64645 4.64645L8 5ZM10 7L10.3536 7.35356C10.5488 7.1583 10.5488 6.84171 10.3536 6.64645L10 7ZM2.5 14.5V15H2.70711L2.85355 14.8536L2.5 14.5ZM0.5 14.5H0C0 14.7761 0.223858 15 0.5 15L0.5 14.5ZM7.5 4.5L7.14645 4.14645L6.79289 4.5L7.14645 4.85356L7.5 4.5ZM10.5858 1.41422L10.2322 1.06066L10.5858 1.41422ZM13.4142 1.41422L13.0607 1.76777L13.0607 1.76777L13.4142 1.41422ZM13.5858 1.58579L13.9393 1.23224L13.9393 1.23224L13.5858 1.58579ZM13.5858 4.41422L13.2322 4.06066L13.5858 4.41422ZM10.5 7.5L10.1464 7.85356L10.5 8.20711L10.8536 7.85356L10.5 7.5ZM0.853553 12.8536L8.35355 5.35356L7.64645 4.64645L0.146447 12.1465L0.853553 12.8536ZM7.64645 5.35356L9.64645 7.35356L10.3536 6.64645L8.35355 4.64645L7.64645 5.35356ZM9.64645 6.64645L2.14645 14.1465L2.85355 14.8536L10.3536 7.35356L9.64645 6.64645ZM2.5 14H0.5V15H2.5V14ZM1 14.5V12.5H0V14.5H1ZM6.14645 3.85356L11.1464 8.85356L11.8536 8.14645L6.85355 3.14645L6.14645 3.85356ZM7.85355 4.85356L10.9393 1.76777L10.2322 1.06066L7.14645 4.14645L7.85355 4.85356ZM13.0607 1.76777L13.2322 1.93934L13.9393 1.23224L13.7678 1.06066L13.0607 1.76777ZM13.2322 4.06066L10.1464 7.14645L10.8536 7.85356L13.9393 4.76777L13.2322 4.06066ZM10.8536 7.14645L7.85355 4.14645L7.14645 4.85356L10.1464 7.85356L10.8536 7.14645ZM13.2322 1.93934C13.818 2.52513 13.818 3.47488 13.2322 4.06066L13.9393 4.76777C14.9157 3.79146 14.9156 2.20855 13.9393 1.23224L13.2322 1.93934ZM10.9393 1.76777C11.5251 1.18198 12.4749 1.18198 13.0607 1.76777L13.7678 1.06066C12.7915 0.0843534 11.2085 0.0843515 10.2322 1.06066L10.9393 1.76777Z" />
            </svg>
          </button>
          <button
            type="button"
            className={`${styles.iconButton} ${styles.squareIconButton}`}
            aria-label={copied ? "Copied HEX" : "Copy HEX"}
            title={copied ? "Copied" : "Copy HEX"}
            onClick={copyHex}
          >
            <span className={`${styles.toolIcon} ${copied ? styles.checkIcon : styles.copyIcon}`} aria-hidden="true" />
          </button>
        </div>
      </div>
      <div className={styles.tonalButtons}>
        <button type="button" onClick={() => onRuleChange("tint")}>Tint</button>
        <button type="button" onClick={() => onRuleChange("shade")}>Shade</button>
        <button type="button" onClick={() => onRuleChange("tone")}>Tone</button>
      </div>
    </div>
  );
}

