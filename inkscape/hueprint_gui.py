...
class ColorWheel(Gtk.DrawingArea):
    ...
    def draw_wheel(self, _widget, cr):
        ...
        if len(points) > 1:
            ...
            recipe_active=hasattr(self.owner,"recipe") and self.owner.recipe.get_active_id() not in (None,"none")
            if len(points)>2 and (recipe_active or self.owner.rule_id() in {"complementary","split_complementary","triadic","square","rectangle_tetradic","polygon"}): cr.close_path()
            cr.stroke()
        ...
        cr.set_source_rgb(*self.owner.background_rgb()); cr.arc(cx,cy,inner-3,0,2*math.pi); cr.fill()
        return False
...
class HuePrintDialog(Gtk.Dialog):
    ...
    def _build(self):
        ...
        swatch_band=Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,spacing=8)
        count_box=Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=3); count_label=Gtk.Label(label="Swatches"); count_label.set_xalign(0); count_box.pack_start(count_label,False,False,0)
        self.count=Gtk.SpinButton.new_with_range(2,8,1); self.count.set_value(5); self.count.connect("value-changed",self.control_changed); count_box.pack_start(self.count,False,False,0)
        swatch_band.pack_start(count_box,False,False,0)
        self.swatches=Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,spacing=5)
        swatch_band.pack_start(self.swatches,True,True,0); left.pack_start(swatch_band,False,False,0)
        ...
        title=Gtk.Label(); title.set_markup("<span size='22000' weight='bold'>HuePrint</span>\nColor harmony studio for Inkscape"); title.set_xalign(0); right.pack_start(title,False,False,6)
        self.theme_toggle=Gtk.ToggleButton(); self.theme_icon=Gtk.Image.new_from_icon_name("weather-clear-night-symbolic",Gtk.IconSize.BUTTON); self.theme_toggle.add(self.theme_icon); self.theme_toggle.set_tooltip_text("Switch to dark mode"); self.theme_toggle.connect("toggled",self.theme_changed); right.pack_start(self.theme_toggle,False,False,0)
        active_line=Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,spacing=6)
        self.active_swatch=Gtk.DrawingArea(); self.active_swatch.set_size_request(38,34); self.active_swatch.connect("draw",self.draw_active_swatch); active_line.pack_start(self.active_swatch,False,False,0)
        self.color_entry=Gtk.Entry(); self.color_entry.set_width_chars(9); self.color_entry.set_text(self.color); self.color_entry.connect("activate",self.entry_changed); active_line.pack_start(self.color_entry,True,True,0)
        choose=Gtk.ColorButton(rgba=_rgba(self.color)); choose.set_title("Choose HuePrint color"); choose.set_tooltip_text("Open color picker"); choose.connect("color-set",self.color_chosen); active_line.pack_start(choose,False,False,0); self.color_button=choose
        copy=Gtk.Button(label="Copy"); copy.connect("clicked",self.copy_color); active_line.pack_start(copy,False,False,0); self.copy_button=copy
        right.pack_start(self._row("Active color",active_line),False,False,0)
        self.harmony=Gtk.ComboBoxText(); [self.harmony.append(i,label) for i,label in HARMONIES]; self.harmony.set_active_id("analogous"); self.harmony.connect("changed",self.harmony_changed); right.pack_start(self._row("Harmony",self.harmony),False,False,0)

        self.recipe=Gtk.ComboBoxText(); [self.recipe.append(i,data[0]) for i,data in RECIPES.items()]; self.recipe.set_wrap_width(2); self.recipe.set_active(0); self.recipe.connect("changed",self.control_changed); right.pack_start(self._row("Palette recipe",self.recipe),False,False,0)
    ...
    def background_rgb(self):
        ...
        return (color.red,color.green,color.blue) if found else ((.12,.12,.14) if self.theme_toggle.get_active() else (.96,.96,.97))
    def theme_changed(self,*_args):
        dark=self.theme_toggle.get_active(); settings=Gtk.Settings.get_default()
        if settings: settings.set_property("gtk-application-prefer-dark-theme",dark)
        self.theme_icon.set_from_icon_name("weather-clear-symbolic" if dark else "weather-clear-night-symbolic",Gtk.IconSize.BUTTON)
        self.theme_toggle.set_tooltip_text("Switch to light mode" if dark else "Switch to dark mode")
        self.wheel.queue_draw()
    def draw_active_swatch(self,widget,cr):
        red,green,blue=[int(self.color[index:index+2],16)/255 for index in (1,3,5)]
    ...
    def copy_color(self,*_args):
        Gtk.Clipboard.get(Gdk.SELECTION_CLIPBOARD).set_text(self.color,-1); self.copy_button.set_label("Copied")
    def harmony_changed(self,*_args):
        if hasattr(self,"recipe") and self.recipe.get_active_id() not in (None,"none"): self.recipe.set_active(0)
        else: self.refresh()
    ...
    def generated_colors(self):
        ...
        count=int(self.count.get_value()) if hasattr(self,"count") else 5
        return generate_recipe(self.color,recipe,count) if recipe and recipe != "none" else generate_palette(self.color,self.rule_id(),count)
